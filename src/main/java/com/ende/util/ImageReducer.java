...
public class ImageReducer {
	private static Logger logger =  LoggerFactory.getLogger("ImageReducer");
	/*** 
	   * 按指定的比例缩放图片 
	   * 
	   ...
	   *      缩放比例，如1.2 
	   */
	  public static void scaleImage(URI sourceImagePath, 
	      URI destinationPath, double scale,String format) { 
	    ...
	    try { 
	      ...
	      logger.error("scaleImage方法压缩图片时出错了"); 
	      e.printStackTrace(); 
	    } 
	   ...
	   */ 
	  public static void scaleImageWithParams(URI sourceImagePath, 
	      URI destinationPath, int width, int height, boolean auto,String format) { 
	    ...
	    ImageIO.write(outputImage, format, new File(destinationPath)); 
	      ...
	      logger.error("scaleImageWithParams方法压缩图片时出错了" + e.getStackTrace()); 
	    } 
