...
var pickout = (function(){
	...
	function createElements(select, index){
		...
		var mgl = document.createElement('span');
		mgl.setAttribute('class',  'pk-span-mgl mgl');

		parent.appendChild(input);
		parent.appendChild(mgl);
	...
	function createOption(data, modal, config){
		...
		icon.setAttribute('class', 'mgl');
		icon.setAttribute('style', 'display: inline-block;');
		icon.innerHTML = data.item.getAttribute('mgl-text') || '';

		var txt = document.createElement('span');
		txt.setAttribute('class', 'txt');
		txt.innerHTML = data.item.innerHTML;
		
		var mgltxt = document.createElement('span');
		mgltxt.setAttribute('class', 'pk-option-mgl mgl');
		mgltxt.setAttribute('style', 'display: inline-block;');
		mgltxt.innerHTML = data.item.attributes["mgltext"].nodeValue;

		main.appendChild(item);
		item.appendChild(icon);
		
		item.appendChild(txt);
		item.appendChild(mgltxt);
		...
		item.addEventListener('click', function(e){
			...
			feedInput(select, txt.innerHTML, mgltxt.innerHTML);		
			closeModal();
		}, false);
	...
	function feedInput(select, value1, value2){
		select.parentElement.querySelector('.pk-input').value = value1;
		select.parentElement.querySelector('.pk-span-mgl').innerHTML = value2;
